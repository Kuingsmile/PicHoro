...
class TencentFileExplorer extends BaseFileExplorer {
  final Map element;
...
class TencentFileExplorerState extends BaseFileExplorerState<TencentFileExplorer> {
  ...
  TencentManageAPI manageAPI = TencentManageAPI();
  ...
  Future<String> getShareUrl(int index) async {
    ...
    return customUrl.isNotEmpty
        ? '$customUrl$filePath'
        : 'https://${widget.element['name']}.cos.${widget.element['location']}.myqcloud.com/$filePath';
  }
  ...
  String getFileDate(int index) {
    return allInfoList[index]['LastModified'] == null
        ? ''
  ...
  @override
  String getPageTitle() => widget.bucketPrefix == '' ? widget.element['name'].split('-')[0] : widget.bucketPrefix;
  ...
  Future<void> _processUploadFiles(List<File> files, bool isSkipImageCheck) async {
    ...
    Global.tencentUploadList = removeDuplicates(Global.tencentUploadList);
  ...
  Future<void> onDownloadButtonPressed() async {
    ...
    String hostPrefix = 'https://${widget.element['name']}.cos.${widget.element['location']}.myqcloud.com/';
    List<String> urlList = [];
    for (int i = 0; i < downloadList.length; i++) {
      urlList.add(hostPrefix + downloadList[i]['Key']);
    }
    Global.tencentDownloadList.addAll(urlList);
    Global.tencentDownloadList = removeDuplicates(Global.tencentDownloadList);
  ...
  void navigateToDownloadManagement() async {
    String downloadPath = await ExternalPath.getExternalStoragePublicDirectory(ExternalPath.DIRECTORY_DOWNLOAD);
    int index = Global.tencentDownloadList.isEmpty ? 0 : 1;
    if (mounted) {
      Application.router
          .navigateTo(context,
