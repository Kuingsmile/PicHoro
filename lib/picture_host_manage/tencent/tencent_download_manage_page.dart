...
class TencentUpDownloadManagePage extends StatefulWidget {
  final String bucketName;
  String downloadPath;
  String tabIndex;
  TencentUpDownloadManagePage(
      {Key? key,
...
class TencentUpDownloadManagePageState
  ...
  _createUploadListItem() {
    List<Widget> list = [];
    for (var i = Global.tencentUploadList.length - 1; i >= 0; i--) {
          ...
          onLongPress: () {
            showCupertinoAlertDialogWithConfirmFunc(
                ...
                onConfirm: () async {
                  ...
                  Global.tencentUploadList.remove(Global.tencentUploadList[i]);
          ...
          child: UploadListItem(
              onUploadPlayPausedPressed: (path, fileName, configMap) async {
                var task = uploadManager
                    .getUpload(jsonDecode(Global.tencentUploadList[i])[1]);
              ...
              path: jsonDecode(Global.tencentUploadList[i])[0],
              fileName: jsonDecode(Global.tencentUploadList[i])[1],
              configMap: jsonDecode(Global.tencentUploadList[i])[2],
  ...
  _createDownloadListItem() {
    List<Widget> list = [];
    for (var i = Global.tencentDownloadList.length - 1; i >= 0; i--) {
      list.add(GestureDetector(
          onLongPress: () {
            showCupertinoAlertDialogWithConfirmFunc(
                context: context,
                ...
                onConfirm: () async {
                  ...
                  Global.tencentDownloadList
                      .remove(Global.tencentDownloadList[i]);
          ...
          child: ListItem(
              onDownloadPlayPausedPressed: (url) async {
                var task =
                    downloadManager.getDownload(Global.tencentDownloadList[i]);
                if (task != null && !task.status.value.isCompleted) {
                  switch (task.status.value) {
              ...
              url: Global.tencentDownloadList[i],
              downloadTask:
                  downloadManager.getDownload(Global.tencentDownloadList[i]))));
    }
    List<Widget> list2 = [
      ...
      Row(
        ...
        children: [
          TextButton(
              onPressed: () async {
                await downloadManager.addBatchDownloads(
                    Global.tencentDownloadList, savedDir);
                setState(() {});
              },
              child: const Text(
                "全部下载",
          ...
          TextButton(
              onPressed: () async {
                await downloadManager
                    .pauseBatchDownloads(Global.tencentDownloadList);
              },
              child: const Text(
                "全部暂停",
                style: TextStyle(
          ...
          TextButton(
              onPressed: () async {
                await downloadManager
                    .resumeBatchDownloads(Global.tencentDownloadList);
              },
              child: const Text(
                "全部继续",
                style: TextStyle(
          ...
          TextButton(
              onPressed: () async {
                await downloadManager
                    .cancelBatchDownloads(Global.tencentDownloadList);
              },
              child: const Text(
                "全部取消",
                style: TextStyle(
      ...
      ValueListenableBuilder(
          valueListenable: downloadManager
              .getBatchDownloadProgress(Global.tencentDownloadList),
          builder: (context, value, child) {
            return Container(
              height: 10,
              margin: const EdgeInsets.symmetric(vertical: 4),
