...
class UpyunUpDownloadManagePage extends StatefulWidget {
  final String bucketName;
  String downloadPath;
  String tabIndex;
  UpyunUpDownloadManagePage(
      {Key? key,
  ...
  @override
  UpyunUpDownloadManagePageState createState() =>
      UpyunUpDownloadManagePageState();
}

class UpyunUpDownloadManagePageState extends State<UpyunUpDownloadManagePage> {
  ...
  _createUploadListItem() {
    List<Widget> list = [];
    for (var i =  Global.upyunUploadList.length - 1; i >= 0; i--) {
          ...
          onLongPress: () {
            showCupertinoAlertDialogWithConfirmFunc(
                ...
                onConfirm: () async {
                  ...
                  Global.upyunUploadList.remove(Global.upyunUploadList[i]);
          ...
          child: UploadListItem(
              onUploadPlayPausedPressed: (path, fileName, configMap) async {
                var task = uploadManager
                    .getUpload(jsonDecode(Global.upyunUploadList[i])[1]);
              ...
              path: jsonDecode(Global.upyunUploadList[i])[0],
              fileName: jsonDecode(Global.upyunUploadList[i])[1],
              configMap: jsonDecode(Global.upyunUploadList[i])[2],
  ...
  _createDownloadListItem() {
    List<Widget> list = [];
    for (var i =  Global.upyunDownloadList.length - 1; i >= 0; i--) {
      list.add(GestureDetector(
          onLongPress: () {
            showCupertinoAlertDialogWithConfirmFunc(
                context: context,
                ...
                onConfirm: () async {
                  Navigator.pop(context);
                  Global.upyunDownloadList.remove(Global.upyunDownloadList[i]);
          ...
          child: ListItem(
              onDownloadPlayPausedPressed: (url) async {
                var task =
                    downloadManager.getDownload(Global.upyunDownloadList[i]);
                if (task != null && !task.status.value.isCompleted) {
                  switch (task.status.value) {
              ...
              onDelete: (url) async {
                ...
                try {
                  ...
                  FLog.error(
                      className: 'UpyunUpDownloadManagePageState',
              ...
              url: Global.upyunDownloadList[i],
              downloadTask:
                  downloadManager.getDownload(Global.upyunDownloadList[i]))));
    }
    List<Widget> list2 = [
        ...
        CupertinoButton(
          ...
          onPressed: () async {
            ...
            externalStorageDirectory =
                '$externalStorageDirectory/PicHoro/Download/upyun';
      ...
      Row(
        ...
        children: [
          TextButton(
              onPressed: () async {
                await downloadManager.addBatchDownloads(
                    Global.upyunDownloadList, savedDir);
                setState(() {});
              },
              child: const Text(
                "全部下载",
          ...
          TextButton(
              onPressed: () async {
                await downloadManager
                    .pauseBatchDownloads(Global.upyunDownloadList);
              },
              child: const Text(
                "全部暂停",
                style: TextStyle(
          ...
          TextButton(
              onPressed: () async {
                await downloadManager
                    .resumeBatchDownloads(Global.upyunDownloadList);
              },
              child: const Text(
                "全部继续",
                style: TextStyle(
          ...
          TextButton(
              onPressed: () async {
                await downloadManager
                    .cancelBatchDownloads(Global.upyunDownloadList);
              },
              child: const Text(
                "全部取消",
                style: TextStyle(
      ...
      ValueListenableBuilder(
          valueListenable: downloadManager
              .getBatchDownloadProgress(Global.upyunDownloadList),
          builder: (context, value, child) {
            return Container(
              height: 10,
              margin: const EdgeInsets.symmetric(vertical: 4),
