...
abstract class BaseDownloadManager {
  ...
  Future<void> download(String url, String savePath, CancelToken cancelToken, {bool forceDownload = false});
  ...
  Future<List<DownloadTask?>> addBatchDownloads(List<String> urls, String savedDir) async {
  ...
  Future<List<DownloadTask?>> addBatchDownloadsWithDirs(List<String> urls, List<String> savedDirList) async {
    ...
    for (var i = 0; i < urls.length; i++) {
      futures.add(addDownload(urls[i], savedDirList[i]));
    }
    return await Future.wait(futures);
  }
  ...
  Future<void> processDownload(String url, String savePath, CancelToken cancelToken, String logTag,
    ...
    try {
      ...
      if (fileExist) {
        ...
        await handlePartialDownload(url, savePath, partialFilePath, partialFile, cancelToken);
      } else {
        await handleNewDownload(url, savePath, partialFilePath, partialFile, cancelToken);
      }
    } catch (e) {
      flogErr(
          e,
  ...
  Future<void> handlePartialDownload(
      String url, String savePath, String partialFilePath, File partialFile, CancelToken cancelToken) async {
    var partialFileLength = await partialFile.length();
    Map<String, dynamic> headers = await getHeaders(url, isPartial: true, partialFileLength: partialFileLength);
  ...
  Future<void> handleNewDownload(
      String url, String savePath, String partialFilePath, File partialFile, CancelToken cancelToken) async {
    Map<String, dynamic> headers = await getHeaders(url);
