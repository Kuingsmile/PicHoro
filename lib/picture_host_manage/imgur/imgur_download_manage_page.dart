...
import 'package:horopic/picture_host_manage/imgur/download_api/imgur_download_task.dart';
...
class ImgurUpDownloadManagePage extends StatefulWidget {
  final String albumName;
  String downloadPath;
  String tabIndex;
  ImgurUpDownloadManagePage(
      {Key? key,
  ...
  @override
  ImgurUpDownloadManagePageState createState() =>
      ImgurUpDownloadManagePageState();
}

class ImgurUpDownloadManagePageState extends State<ImgurUpDownloadManagePage> {
  ...
  void initState() {
    ...
    if (Global.imgurUploadList.isNotEmpty) {
      for (var i = 0; i < Global.imgurUploadList.length; i++) {
        var currentElement = jsonDecode(Global.imgurUploadList[i]);
          ...
          onLongPress: () {
            showCupertinoAlertDialogWithConfirmFunc(
                ...
                onConfirm: () async {
                  ...
                  Global.imgurUploadList.remove(Global.imgurUploadList[i]);
          ...
          child: UploadListItem(
              onUploadPlayPausedPressed: (path, fileName, configMap) async {
                var task = uploadManager
                    .getUpload(jsonDecode(Global.imgurUploadList[i])[1]);
              ...
              path: jsonDecode(Global.imgurUploadList[i])[0],
              fileName: jsonDecode(Global.imgurUploadList[i])[1],
              configMap: jsonDecode(Global.imgurUploadList[i])[2],
      ...
      list.add(GestureDetector(
          onLongPress: () {
            showCupertinoAlertDialogWithConfirmFunc(
                ...
                onConfirm: () async {
                  Navigator.pop(context);
                  Global.imgurDownloadList.remove(Global.imgurDownloadList[i]);
                  await Global.setImgurDownloadList(Global.imgurDownloadList);
                  setState(() {});
                });
          },
          child: ListItem(
              onDownloadPlayPausedPressed: (url) async {
                var task =
                    downloadManager.getDownload(Global.imgurDownloadList[i]);
                if (task != null && !task.status.value.isCompleted) {
                  switch (task.status.value) {
              ...
              onDelete: (url) async {
                ...
                try {
                  ...
                  FLog.error(
                      className: 'imgurUpDownloadManagePageState',
              ...
              url: Global.imgurDownloadList[i],
              downloadTask:
                  downloadManager.getDownload(Global.imgurDownloadList[i]))));
    }
    List<Widget> list2 = [
        ...
        CupertinoButton(
          ...
          onPressed: () async {
            ...
            externalStorageDirectory =
                '$externalStorageDirectory/PicHoro/Download/imgur';
      ...
      Row(
        ...
        children: [
          TextButton(
              onPressed: () async {
                await downloadManager.addBatchDownloads(
                    Global.imgurDownloadList, savedDir);
                setState(() {});
              },
              child: const Text(
                "全部下载",
          ...
          TextButton(
              onPressed: () async {
                await downloadManager
                    .pauseBatchDownloads(Global.imgurDownloadList);
              },
              child: const Text(
                "全部暂停",
                style: TextStyle(
          ...
          TextButton(
              onPressed: () async {
                await downloadManager
                    .resumeBatchDownloads(Global.imgurDownloadList);
              },
              child: const Text(
                "全部继续",
                style: TextStyle(
          ...
          TextButton(
              onPressed: () async {
                await downloadManager
                    .cancelBatchDownloads(Global.imgurDownloadList);
              },
              child: const Text(
                "全部取消",
                style: TextStyle(
      ...
      ValueListenableBuilder(
          valueListenable: downloadManager
              .getBatchDownloadProgress(Global.imgurDownloadList),
          builder: (context, value, child) {
            return Container(
              height: 10,
              margin: const EdgeInsets.symmetric(vertical: 4),
