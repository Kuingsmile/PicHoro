...
import 'package:horopic/picture_host_manage/webdav/download_api/webdav_download_task.dart';
...
import 'package:horopic/picture_host_manage/webdav/upload_api/webdav_upload_task.dart';
...
class WebdavUpDownloadManagePage extends StatefulWidget {
  final String bucketName;
  String downloadPath;
  String tabIndex;
  WebdavUpDownloadManagePage(
      {Key? key,
  ...
  @override
  WebdavUpDownloadManagePageState createState() =>
      WebdavUpDownloadManagePageState();
}

class WebdavUpDownloadManagePageState
    extends State<WebdavUpDownloadManagePage> {
  ...
  void initState() {
    ...
    savedDir =
        '${widget.downloadPath}/PicHoro/Download/webdav/${widget.bucketName}/';
    if (Global.webdavUploadList.isNotEmpty) {
      for (var i = 0; i < Global.webdavUploadList.length; i++) {
        var currentElement = jsonDecode(Global.webdavUploadList[i]);
  ...
  _createUploadListItem() {
    List<Widget> list = [];
    for (var i = Global.webdavUploadList.length - 1; i >= 0; i--) {
          ...
          onLongPress: () {
            showCupertinoAlertDialogWithConfirmFunc(
                ...
                onConfirm: () async {
                  ...
                  Global.webdavUploadList.remove(Global.webdavUploadList[i]);
          ...
          child: UploadListItem(
              onUploadPlayPausedPressed: (path, fileName, configMap) async {
                var task = uploadManager
                    .getUpload(jsonDecode(Global.webdavUploadList[i])[1]);
              ...
              path: jsonDecode(Global.webdavUploadList[i])[0],
              fileName: jsonDecode(Global.webdavUploadList[i])[1],
              configMap: jsonDecode(Global.webdavUploadList[i])[2],
  ...
  _createDownloadListItem() {
    List<Widget> list = [];
    for (var i = Global.webdavDownloadList.length - 1; i >= 0; i--) {
      list.add(GestureDetector(
          onLongPress: () {
            showCupertinoAlertDialogWithConfirmFunc(
                context: context,
                ...
                onConfirm: () async {
                  ...
                  Global.webdavDownloadList
                      .remove(Global.webdavDownloadList[i]);
          ...
          child: ListItem(
              onDownloadPlayPausedPressed: (url) async {
                var task =
                    downloadManager.getDownload(Global.webdavDownloadList[i]);
                if (task != null && !task.status.value.isCompleted) {
                  switch (task.status.value) {
              ...
              onDelete: (url) async {
                ...
                try {
                  ...
                  FLog.error(
                      className: 'WebdavUpDownloadManagePageState',
              ...
              url: Global.webdavDownloadList[i],
              downloadTask:
                  downloadManager.getDownload(Global.webdavDownloadList[i]))));
    }
    List<Widget> list2 = [
        ...
        CupertinoButton(
          ...
          onPressed: () async {
            ...
            externalStorageDirectory =
                '$externalStorageDirectory/PicHoro/Download/webdav';
      ...
      Row(
        ...
        children: [
          TextButton(
              onPressed: () async {
                await downloadManager.addBatchDownloads(
                    Global.webdavDownloadList, savedDir);
                setState(() {});
              },
              child: const Text(
                "全部下载",
          ...
          TextButton(
              onPressed: () async {
                await downloadManager
                    .pauseBatchDownloads(Global.webdavDownloadList);
              },
              child: const Text(
                "全部暂停",
                style: TextStyle(
          ...
          TextButton(
              onPressed: () async {
                await downloadManager
                    .resumeBatchDownloads(Global.webdavDownloadList);
              },
              child: const Text(
                "全部继续",
                style: TextStyle(
          ...
          TextButton(
              onPressed: () async {
                await downloadManager
                    .cancelBatchDownloads(Global.webdavDownloadList);
              },
              child: const Text(
                "全部取消",
                style: TextStyle(
      ...
      ValueListenableBuilder(
          valueListenable: downloadManager
              .getBatchDownloadProgress(Global.webdavDownloadList),
          builder: (context, value, child) {
            return Container(
              height: 10,
              margin: const EdgeInsets.symmetric(vertical: 4),
