...
import 'package:horopic/picture_host_manage/lskypro/download_api/lskypro_download_task.dart';
...
class LskyproUpDownloadManagePage extends StatefulWidget {
  final String albumName;
  String downloadPath;
  String tabIndex;
  LskyproUpDownloadManagePage(
      {Key? key,
  ...
  @override
  LskyproUpDownloadManagePageState createState() =>
      LskyproUpDownloadManagePageState();
}

class LskyproUpDownloadManagePageState
    extends State<LskyproUpDownloadManagePage> {
  ...
  void initState() {
    ...
    if (Global.lskyproUploadList.isNotEmpty) {
      for (var i = 0; i < Global.lskyproUploadList.length; i++) {
        var currentElement = jsonDecode(Global.lskyproUploadList[i]);
      ...
      list.add(GestureDetector(
            ...
            showCupertinoAlertDialogWithConfirmFunc(
                ...
                onConfirm: () async {
                  ...
                  Global.lskyproDownloadList
                      .remove(Global.lskyproDownloadList[i]);
          ...
          child: ListItem(
              onDownloadPlayPausedPressed: (url) async {
                var task =
                    downloadManager.getDownload(Global.lskyproDownloadList[i]);
                if (task != null && !task.status.value.isCompleted) {
                  switch (task.status.value) {
              ...
              onDelete: (url) async {
                ...
                try {
                  ...
                  FLog.error(
                      className: 'lskyproUpDownloadManagePageState',
              ...
              url: Global.lskyproDownloadList[i],
              downloadTask:
                  downloadManager.getDownload(Global.lskyproDownloadList[i]))));
    }
    List<Widget> list2 = [
      ...
      Row(
        ...
        children: [
          TextButton(
              onPressed: () async {
                await downloadManager.addBatchDownloads(
                    Global.lskyproDownloadList, savedDir);
                setState(() {});
              },
              child: const Text(
                "全部下载",
          ...
          TextButton(
              onPressed: () async {
                await downloadManager
                    .pauseBatchDownloads(Global.lskyproDownloadList);
              },
              child: const Text(
                "全部暂停",
                style: TextStyle(
          ...
          TextButton(
              onPressed: () async {
                await downloadManager
                    .resumeBatchDownloads(Global.lskyproDownloadList);
              },
              child: const Text(
                "全部继续",
                style: TextStyle(
          ...
          TextButton(
              onPressed: () async {
                await downloadManager
                    .cancelBatchDownloads(Global.lskyproDownloadList);
              },
              child: const Text(
                "全部取消",
                style: TextStyle(
      ...
      ValueListenableBuilder(
          valueListenable: downloadManager
              .getBatchDownloadProgress(Global.lskyproDownloadList),
          builder: (context, value, child) {
            return Container(
              height: 10,
              margin: const EdgeInsets.symmetric(vertical: 4),
