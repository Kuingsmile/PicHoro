...
import 'package:horopic/picture_host_manage/qiniu/upload_api/qiniu_upload_task.dart';
...
class QiniuUpDownloadManagePage extends StatefulWidget {
  final String bucketName;
  String downloadPath;
  String tabIndex;
  QiniuUpDownloadManagePage(
      {Key? key,
  ...
  @override
  QiniuUpDownloadManagePageState createState() =>
      QiniuUpDownloadManagePageState();
}

class QiniuUpDownloadManagePageState extends State<QiniuUpDownloadManagePage> {
          ...
          onLongPress: () {
            showCupertinoAlertDialogWithConfirmFunc(
                ...
                onConfirm: () async {
                  ...
                  Global.qiniuUploadList.remove(Global.qiniuUploadList[i]);
          ...
          child: UploadListItem(
              onUploadPlayPausedPressed: (path, fileName, configMap) async {
                var task = uploadManager
                    .getUpload(jsonDecode(Global.qiniuUploadList[i])[1]);
              ...
              path: jsonDecode(Global.qiniuUploadList[i])[0],
              fileName: jsonDecode(Global.qiniuUploadList[i])[1],
              configMap: jsonDecode(Global.qiniuUploadList[i])[2],
      ...
      list.add(GestureDetector(
          onLongPress: () {
            showCupertinoAlertDialogWithConfirmFunc(
                ...
                onConfirm: () async {
                  Navigator.pop(context);
                  Global.qiniuDownloadList.remove(Global.qiniuDownloadList[i]);
          ...
          child: ListItem(
              onDownloadPlayPausedPressed: (url) async {
                var task =
                    downloadManager.getDownload(Global.qiniuDownloadList[i]);
                if (task != null && !task.status.value.isCompleted) {
                  switch (task.status.value) {
              ...
              url: Global.qiniuDownloadList[i],
              downloadTask:
                  downloadManager.getDownload(Global.qiniuDownloadList[i]))));
    }
    List<Widget> list2 = [
        ...
        CupertinoButton(
          ...
          onPressed: () async {
            ...
            externalStorageDirectory =
                '$externalStorageDirectory/PicHoro/Download/qiniu';
      ...
      Row(
        ...
        children: [
          TextButton(
              onPressed: () async {
                await downloadManager.addBatchDownloads(
                    Global.qiniuDownloadList, savedDir);
                setState(() {});
              },
              child: const Text(
                "全部下载",
          ...
          TextButton(
              onPressed: () async {
                await downloadManager
                    .pauseBatchDownloads(Global.qiniuDownloadList);
              },
              child: const Text(
                "全部暂停",
                style: TextStyle(
          ...
          TextButton(
              onPressed: () async {
                await downloadManager
                    .resumeBatchDownloads(Global.qiniuDownloadList);
              },
              child: const Text(
                "全部继续",
                style: TextStyle(
          ...
          TextButton(
              onPressed: () async {
                await downloadManager
                    .cancelBatchDownloads(Global.qiniuDownloadList);
              },
              child: const Text(
                "全部取消",
                style: TextStyle(
      ...
      ValueListenableBuilder(
          valueListenable: downloadManager
              .getBatchDownloadProgress(Global.qiniuDownloadList),
          builder: (context, value, child) {
            return Container(
              height: 10,
              margin: const EdgeInsets.symmetric(vertical: 4),
